    ...
    create({ sizeList, colorList, referenceList, data }) {
        ...
        containerBtn.querySelector("#img3DBtn").addEventListener("click", () => {
            ...
            if (referenceList) { console.log('estoy aqui') }
                ...
                const modal = new ModalMudi(data, this.color);
...
let counterInitExperience=0;
const initExperience = () =>{

    const numberSku = document.querySelector('.sku.sku-value');
    const father = document.querySelector('.NewProductImages__container');

    if(counterInitExperience==300){
        return;
    }
    else if(!numberSku || !father){
        counterInitExperience++
        requestAnimationFrame( initExperience );
    }
    else{
        mudiExperience.experienceOn(numberSku.innerHTML, father);
    }
